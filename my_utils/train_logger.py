...
from my_utils import main_utils
from datetime import datetime
from torch.optim import Optimizer
from my_utils.types import PathT, InputSample
...
class TrainLogger:
    ...
    def write_epoch_statistics(self, epoch: int, epoch_time: float,
        ...
        text += '\n'
        text += 'c loss: %.6f,   ' % train_c_loss
        text += 'acc: %.3f,   ' % train_acc
        text += 'iou: %.3f,   ' % train_iou
        text += '\n'
        text += 'c loss: %.6f,   ' % eval_c_loss
        text += 'acc: %.3f,   ' % eval_acc
        text += 'iou: %.3f   ' % eval_iou
