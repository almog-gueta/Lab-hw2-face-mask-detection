...
DATA_TRANSFORMS = {
    'train' : transforms.Compose([
        transforms.Resize(256) ,#RandomResizedCrop(224), #transforms.Resize((64,64),interpolation=Image.BICUBIC),
        # transforms.RandomHorizontalFlip(),
        transforms.CenterCrop(224),
        transforms.ToTensor(),
        transforms.Normalize([0.5244, 0.4904, 0.4781],[0.2642, 0.2608, 0.2561]) #([0.1612, 0.1093, 0.2747], [1.0834, 1.0877, 1.0614])([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ]),
    'eval' : transforms.Compose([
        transforms.Resize(256), #transforms.Resize((64, 64), interpolation=Image.BICUBIC),
        transforms.CenterCrop(224), #todo- shir doesnt have it
        transforms.ToTensor(),
...
class FaceMaskDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not self.is_predict:
            ...
            item = (image, bbox, proper_mask,w_scale,h_scale) #todo remember this when predict!
        return item
